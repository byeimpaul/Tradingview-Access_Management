...
class TradingView:
    def __init__(self):
        ...
        print(test.text)
        print('sessionid from database: ' + self.sessionid)
        if test.status_code != 200:
            print('session id from database is invalid')
            ...
            self.save_session_id(self.sessionid)

    def get_session_id(self):
        self.cursor.execute("SELECT sessionid FROM sessions LIMIT 1;")
        row = self.cursor.fetchone()
        return row[0] if row else 'abcd'

    def save_session_id(self, sessionid):
        self.cursor.execute("INSERT INTO sessions (sessionid) VALUES (%s) ON CONFLICT (sessionid) DO NOTHING;", (sessionid,))
